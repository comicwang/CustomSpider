...
namespace CustomSpider
{
    ...
    public class ContentManger
    {
        ...
        public static void View(string forlder, SaveType saveType, string fileName)
        {
            switch (saveType)
            {
                case SaveType.文本内容:
                    break;
                case SaveType.文件:
                    if (string.IsNullOrEmpty(forlder) || !Directory.Exists(forlder))
                        return;
                    string filePath = Directory.GetFiles(forlder, fileName, SearchOption.AllDirectories).FirstOrDefault();
                    if (filePath == null)
                        return;
